...
class Pico:
    def __init__(self,async=False):
        ...
        if not async:
            self.handle=self.picolog.HRDLOpenUnit()#OK
        else:
            self.handle=self.picolog.HRDLOpenUnitAsync()#not tested
        if self.handle < 1: raise RuntimeError("Picolog failed to open. Try to turn it off and on again.")
    ...
    def closeUnit(self):#do not use. device is unable to start again. Just leave it at the end of meas.
        '''
            ...
            0 if not
        '''
        print self.picolog.HRDLCloseUnit(self.handle)
    
    def openUnitProgress(self):#not tested
        progress=c_short()
        r = self.picolog.HRDLOpenUnitProgress(self.handle,byref(progress))
    ...
    def collectSingleValueAsync(self,channel,rng,conversionTime,singleEnded):#not tested
        ...
        print self.picolog.HRDLCollectSingleValueAsync(self.handle,
                                                        channel,rng,conversionTime,singleEnded)

    def getMinMaxAdcCounts(self,channel):#OK
        minAdc=c_long()
        maxAdc=c_long()
        
        r=self.picolog.HRDLGetMinMaxAdcCounts(self.handle,
                                              byref(minAdc),byref(maxAdc),channel)
        if r==0: raise RuntimeError('Check the device.')
        """
    ...
    def getNumberOfEnabledChannels(self):#not tested
        nEnabledChannels=c_short()
        r = self.picolog.HRDLGetNumberOfEnabledChannels(self.handle,byref(nEnabledChannels))
    ...
    def getSingleValue(self,channel,rng,conversionTime,singleEnded): #OK
        ################### reconfugures analog channel settings #############################    
        overflow = c_short()
        val = c_long()
        r=self.picolog.HRDLGetSingleValue(self.handle,
                                          channel,rng,conversionTime,
                                          singleEnded,byref(overflow),byref(val))
        if r==0: raise RuntimeError('Check the device.')
        '''
    ...
    def getSingleValueAsync(self):#not tested
        val=c_long()
        overflow=c_short()
        r=self.picolog.HRDLGetSingleValueAsync(self.handle,byref(value),byref(overflow))
    ...
    def getTimesAndValues(self,noOfValues):#not tested
        times=c_long()
        vals=c_long()
        overflow=c_long()
        r=self.picolog.HRDLGetTimesAndValues (self.handle,
                                              byref(times),byref(vals),byref(overflow),
                                              noOfValues )
    ...
    def getUnitInfo(self,info):# OK

        string= create_string_buffer(100)
        r=self.picolog.HRDLGetUnitInfo(self.handle,string,100,info)
        #print r
        #if r==0: raise RuntimeError('stringLength too short')#CRASHES BEFORE
        '''
    ...
    def getValues(self,noOfValues):#not tested
        overflow=c_short()
        vals=c_long()
        r=self.picolog.HRDLGetValues(self.handle,byref(vals),byref(overflow),noOfValues )
    ...
    def run(self,nValues,method):#not tested
        ...
        print self.picolog.HRDLRun(handle,c_long(nValues),method)


    def setAnalogInChannel(self,channel,enabled,rng,singleEnded):#not tested
        ...
        print self.picolog.HRDLSetAnalogInChannel(self.handle,channel,enabled,rng,singleEnded)

    def setDigitalIOChannel(self,directionOut,digitalOutPinState,enabledDigitalIn): #not tested
        ...
        """
        print self.picolog.HRDLSetDigitalIOChannel(self.handle,
                                                   directionOut,digitalOutPinState,
                                                   enabledDigitalIn)

    def setInterval(self,sampleInterval_ms,conversionTime ):#not tested
        ...
        print self.picolog.HRDLSetInterval(self.handle,sampleInterval_ms,conversionTime )

   
    
    def setMains(self,reject):#OK
    ...
    def stop(self):#not tested
        print self.picolog.HRDLStop(self.handle)
